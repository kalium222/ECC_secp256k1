...
def encrypt(plaintext: str, key: str = ""):
    if not key:
        proc = subprocess.run(f"./g2 --encrypt '{plaintext}'", shell=True, capture_output=True)
    else:
        proc = subprocess.run(f"./g2 --encrypt '{plaintext}' --key <(echo '{key}')", shell=True, executable="/bin/bash", capture_output=True)
    if proc.returncode != 0:
        raise Exception("Encryption failed to run")
    return verify(proc.stdout.decode().rstrip('\n'), "Encryption generated invalid output")

def decrypt(ciphertext: str, key: str = ""):
    if not key:
        proc = subprocess.run(f"./g2 --decrypt '{ciphertext}'", shell=True, capture_output=True)
    else:
        proc = subprocess.run(f"./g2 --decrypt '{ciphertext}' --key <(echo '{key}')", shell=True, executable="/bin/bash", capture_output=True)
    if proc.returncode != 0:
        raise Exception("Decryption failed to run")
    result = proc.stdout.decode().rstrip('\n')
    return result if result == cheater else verify(result, "Decryption generated invalid output")
...
def get_challenge_plaintext():
    try:
        cpt = open("plaintext.txt").read().rstrip('\n')
        if not cpt:
...
def get_challenge_ciphertext():
    try:
        cct = open("ciphertext.txt").read().rstrip('\n')
        if not cct:
...
def do_tests():
    ...
    cpt = get_challenge_plaintext()
    print("Success!")

    print("Fetching challenge ciphertext... ", end='')
    cct = get_challenge_ciphertext()
    print("Success!")
